...
type ServiceRow = { id?: string; name: string; priceCategoryId: string | null };
...
export function DirectionsManager() {
  ...
  const openEdit = (d: DirectionWithServices) => {
    ...
    setServiceList(
      d.services.map((s) => ({ id: s.id, name: s.name, priceCategoryId: s.priceCategoryId ?? null })),
    );
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        directionId = editing.id;
        const origIds = editing.services.map((s) => s.id);
        const keptIds = serviceList.filter((s) => s.id).map((s) => s.id);
        for (const id of origIds) {
          if (!keptIds.includes(id)) await apiRequest("DELETE", `/api/admin/services/${id}`);
        }
        // update price-category links for kept services
        const origById = new Map(editing.services.map((s) => [s.id, s] as const));
        for (const s of serviceList) {
          if (s.id && origById.get(s.id)?.priceCategoryId !== s.priceCategoryId) {
            await apiRequest("PATCH", `/api/admin/services/${s.id}`, {
              priceCategoryId: s.priceCategoryId,
            });
          }
        }
      } else {
        const res = await apiRequest("POST", "/api/admin/directions", form);
        directionId = (await res.json()).id;
      }
      // create new services (those without id)
  ...
  const remove = async (id: string) => {
    if (!confirm("Удалить направление со всеми услугами?")) return;
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Направления и услуги</h1>
          <p className="mt-1 text-sm text-gray-500">Категории услуг и вложенные услуги</p>
        </div>
        <Button onClick={openCreate} data-testid="button-create-direction">
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label>Услуги</Label>
              <p className="text-xs text-gray-400">
                Для каждой услуги можно выбрать категорию прайс-листа — тогда на
                странице услуги покажется список процедур с ценами.
              </p>
              <div className="space-y-2">
                {serviceList.map((s, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-lg border border-gray-100 bg-gray-50 p-2">
                    <span className="flex-1 truncate px-1 text-sm text-gray-800">{s.name}</span>
                    <select
                      value={s.priceCategoryId ?? ""}
                      onChange={(e) =>
                        setServiceList(
                          serviceList.map((row, idx) =>
                            idx === i ? { ...row, priceCategoryId: e.target.value || null } : row,
                          ),
                        )
                      }
                      data-testid={`select-price-category-${i}`}
                      className="h-8 max-w-[55%] rounded-md border border-gray-200 bg-white px-2 text-xs text-gray-700"
                    >
                      <option value="">Без прайса</option>
                      {priceCategories.map((c) => (
                        <option key={c.id} value={c.id}>
                    ...
                    <button
                      type="button"
                      onClick={() => setServiceList(serviceList.filter((_, idx) => idx !== i))}
                ...
                <Input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && newService.trim()) {
                      e.preventDefault();
                      setServiceList([...serviceList, { name: newService.trim(), priceCategoryId: null }]);
                      setNewService("");
                    }
                  }}
